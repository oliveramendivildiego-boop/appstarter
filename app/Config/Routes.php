<?php

use CodeIgniter\Router\RouteCollection;

/**
 * @var RouteCollection $routes
 */
 
 
$routes->get('/', 'Home::index');
$routes->get('home', 'Home::index');
$routes->get('home/tenant-broadcast-image', 'Home::tenantBroadcastImage');
$routes->get('subscription-blocked', 'Home::subscriptionBlocked');
$routes->get('tenant-subscription', 'TenantSubscription::index');
$routes->get('tenant-subscription/voucher/(:num)', 'TenantSubscription::voucher/$1');
$routes->get('no_access/(:any)', 'NoAccess::index/$1');
$routes->get('home/logout', 'Home::logout');
$routes->get('account/password', 'Account::password');
$routes->post('account/password', 'Account::updatePassword');
$routes->get('status/checkEmployeeActive', 'StatusCheck::checkEmployeeActive');
$routes->post('status/checkEmployeeActive', 'StatusCheck::checkEmployeeActive');
$routes->get('login', 'Login::index');
$routes->post('login', 'Login::index');
$routes->get('login/tenantHandoff/(:segment)', 'Login::tenantHandoff/$1');

// Doctor portal (login con usuario/contraseña de doctor)
$routes->get('doctor/home', 'DoctorHome::index');
$routes->get('doctor/expediente/(:num)', 'DoctorHome::expediente/$1');
$routes->get('doctor/history/(:num)', 'DoctorHome::history/$1');
$routes->get('doctor/expediente_chart/(:num)', 'DoctorHome::expedienteChart/$1');
$routes->get('doctor/viewreport/(:num)', 'DoctorHome::viewreport/$1');
$routes->get('doctor/pdf/(:num)', 'DoctorHome::pdf/$1');
$routes->post('doctor/search', 'DoctorHome::search');
$routes->get('doctor/logout', 'DoctorHome::logout');
$routes->get('qr/generate', 'Qr::generate');
$routes->get('resultados/(:segment)/pdf', 'PublicResultados::pdf/$1');
$routes->get('resultados/(:segment)', 'PublicResultados::view/$1');
$routes->post('login/google_login', 'Login::googleLogin');

// Customers
$routes->get('customers', 'Customers::index');

// Módulos placeholder (en desarrollo) - sin controlador completo
$routes->get('item_kits', 'ItemKits::index');
$routes->get('items', 'ModulePlaceholder::index');
$routes->get('sales', 'ModulePlaceholder::index');
$routes->get('suppliers', 'ModulePlaceholder::index');
$routes->get('receivings', 'ModulePlaceholder::index');
$routes->get('reports', 'Reports::index');
$routes->get('reportes', 'Reports::index');
$routes->get('reports/cotizacionesGuardadas', 'Reports::cotizacionesGuardadas');
$routes->get('reports/cotizacionesGuardadasPdf', 'Reports::cotizacionesGuardadasPdf');
$routes->get('reports/registrosFecha', 'Reports::registrosFecha');
$routes->get('reports/registrosFechaPdf', 'Reports::registrosFechaPdf');
$routes->get('reports/ingresosFecha', 'Reports::ingresosFecha');
$routes->get('reports/ingresosFechaPdf', 'Reports::ingresosFechaPdf');
$routes->get('reports/porDoctor', 'Reports::porDoctor');
$routes->get('reports/porDoctorPdf', 'Reports::porDoctorPdf');
$routes->get('reports/pagos', 'Reports::pagos');
$routes->get('reports/pagosDetallePorTipo', 'Reports::pagosDetallePorTipo');
$routes->get('reports/pagosDetallePorProcesamiento', 'Reports::pagosDetallePorProcesamiento');
$routes->get('reports/pagosPdf', 'Reports::pagosPdf');
$routes->get('reports/pagosPendientes', 'Reports::pagosPendientes');
$routes->get('reports/pagosPendientesPdf', 'Reports::pagosPendientesPdf');
$routes->get('reports/ingresosPorTipoProcesamiento', 'Reports::ingresosPorTipoProcesamiento');
$routes->get('reports/ingresosPorTipoProcesamientoPdf', 'Reports::ingresosPorTipoProcesamientoPdf');
$routes->get('reports/ingresosPorTipoProcesamientoExcel', 'Reports::ingresosPorTipoProcesamientoExcel');
$routes->get('reports/pagosCierres', 'Reports::pagosCierres');
$routes->get('reports/pagosCierresPdf', 'Reports::pagosCierresPdf');
$routes->post('reports/pagosCierreCrear', 'Reports::pagosCierreCrear');
$routes->get('reports/pagosCierre/(:num)', 'Reports::pagosCierreVer/$1');
$routes->get('reports/pagosCierrePdf/(:num)', 'Reports::pagosCierrePdf/$1');
$routes->get('reports/pruebasFecha', 'Reports::pruebasFecha');
$routes->get('reports/pruebasFechaPdf', 'Reports::pruebasFechaPdf');
$routes->get('reports/pruebasDetallado', 'Reports::pruebasDetallado');
$routes->get('reports/pruebasDetalladoDetalle', 'Reports::pruebasDetalladoDetalle');
$routes->get('reports/pruebasDetalladoPdf', 'Reports::pruebasDetalladoPdf');
$routes->get('reports/pruebasIncompletasFecha', 'Reports::pruebasIncompletasFecha');
$routes->get('reports/pruebasIncompletasFechaPdf', 'Reports::pruebasIncompletasFechaPdf');
$routes->get('reports/pruebasAnuladasFecha', 'Reports::pruebasAnuladasFecha');
$routes->get('reports/pruebasAnuladasFechaPdf', 'Reports::pruebasAnuladasFechaPdf');
$routes->get('reports/estadisticasLaboratorio', 'Reports::estadisticasLaboratorio');
$routes->get('reports/estadisticasLaboratorioPdf', 'Reports::estadisticasLaboratorioPdf');
$routes->get('reports/pruebasPorGrupoAnalisis', 'Reports::pruebasPorGrupoAnalisis');
$routes->get('reports/pruebasPorGrupoAnalisisPdf', 'Reports::pruebasPorGrupoAnalisisPdf');
$routes->get('reportes/pruebasPorGrupoAnalisis', 'Reports::pruebasPorGrupoAnalisis');
$routes->get('reports/insumosVencimiento', 'Reports::insumosVencimiento');
$routes->get('reports/insumosVencimientoPdf', 'Reports::insumosVencimientoPdf');
$routes->get('reports/inventarioKardex', 'Reports::inventarioKardex');
$routes->get('reports/costosPruebas', 'Reports::costosPruebas');
$routes->get('reports/editarCostosPruebas', 'Reports::editarCostosPruebas');
$routes->post('reports/saveCostosPruebas', 'Reports::saveCostosPruebas');
$routes->get('reports/exportCostosPruebasImport', 'Reports::exportCostosPruebasImport');
$routes->post('reports/importCostosPruebas', 'Reports::importCostosPruebas');
$routes->get('reports/costosPruebasPdf', 'Reports::costosPruebasPdf');
$routes->get('reports/exportCostosPruebas', 'Reports::exportCostosPruebas');
$routes->get('reports/valoresReferencia', 'Reports::valoresReferencia');
$routes->get('reports/valoresReferenciaPdf', 'Reports::valoresReferenciaPdf');
$routes->get('reports/exportValoresReferencia', 'Reports::exportValoresReferencia');
$routes->get('reports/exportValoresReferenciaImport', 'Reports::exportValoresReferenciaImport');
$routes->post('reports/importValoresReferencia', 'Reports::importValoresReferencia');
$routes->get('reports/catalogoPruebas', 'Reports::catalogoPruebas');
$routes->get('reports/catalogoPruebasPdf', 'Reports::catalogoPruebasPdf');
$routes->get('reports/catalogoPruebasExcel', 'Reports::catalogoPruebasExcel');
// --- Reportes analíticos nuevos (controlador independiente ReportsAnalytics) ---
$routes->get('reports/pruebasMasSolicitadas', 'ReportsAnalytics::pruebasMasSolicitadas');
$routes->get('reports/pruebasMasSolicitadasPdf', 'ReportsAnalytics::pruebasMasSolicitadasPdf');
$routes->get('reports/pruebasMasSolicitadasExcel', 'ReportsAnalytics::pruebasMasSolicitadasExcel');
$routes->get('reports/tendenciaPaciente', 'ReportsAnalytics::tendenciaPaciente');
$routes->get('reports/tendenciaPacientePdf', 'ReportsAnalytics::tendenciaPacientePdf');
$routes->get('reports/tendenciaPacienteExcel', 'ReportsAnalytics::tendenciaPacienteExcel');
$routes->get('reports/valoresCriticos', 'ReportsAnalytics::valoresCriticos');
$routes->get('reports/valoresCriticosPdf', 'ReportsAnalytics::valoresCriticosPdf');
$routes->get('reports/valoresCriticosExcel', 'ReportsAnalytics::valoresCriticosExcel');
$routes->get('reports/historialPrueba', 'ReportsAnalytics::historialPrueba');
$routes->get('reports/historialPruebaPdf', 'ReportsAnalytics::historialPruebaPdf');
$routes->get('reports/historialPruebaExcel', 'ReportsAnalytics::historialPruebaExcel');
$routes->get('reports/tiempoEntrega', 'ReportsAnalytics::tiempoEntrega');
$routes->get('reports/tiempoEntregaPdf', 'ReportsAnalytics::tiempoEntregaPdf');
$routes->get('reports/tiempoEntregaExcel', 'ReportsAnalytics::tiempoEntregaExcel');
$routes->get('reports/productividadUsuarios', 'ReportsAnalytics::productividadUsuarios');
$routes->get('reports/productividadUsuariosPdf', 'ReportsAnalytics::productividadUsuariosPdf');
$routes->get('reports/productividadUsuariosExcel', 'ReportsAnalytics::productividadUsuariosExcel');
$routes->get('reports/notificacionesEntrega', 'ReportsAnalytics::notificacionesEntrega');
$routes->get('reports/notificacionesEntregaPdf', 'ReportsAnalytics::notificacionesEntregaPdf');
$routes->get('reports/notificacionesEntregaExcel', 'ReportsAnalytics::notificacionesEntregaExcel');
$routes->get('reports/resultadosCorregidos', 'ReportsAnalytics::resultadosCorregidos');
$routes->get('reports/resultadosCorregidosPdf', 'ReportsAnalytics::resultadosCorregidosPdf');
$routes->get('reports/resultadosCorregidosExcel', 'ReportsAnalytics::resultadosCorregidosExcel');
$routes->get('reports/pendientesValidacion', 'ReportsAnalytics::pendientesValidacion');
$routes->get('reports/pendientesValidacionPdf', 'ReportsAnalytics::pendientesValidacionPdf');
$routes->get('reports/pendientesValidacionExcel', 'ReportsAnalytics::pendientesValidacionExcel');
$routes->get('reports/consumoInsumos', 'ReportsAnalytics::consumoInsumos');
$routes->get('reports/consumoInsumosPdf', 'ReportsAnalytics::consumoInsumosPdf');
$routes->get('reports/consumoInsumosExcel', 'ReportsAnalytics::consumoInsumosExcel');
$routes->get('reports/proyeccionInsumos', 'ReportsAnalytics::proyeccionInsumos');
$routes->get('reports/proyeccionInsumosPdf', 'ReportsAnalytics::proyeccionInsumosPdf');
$routes->get('reports/proyeccionInsumosExcel', 'ReportsAnalytics::proyeccionInsumosExcel');
$routes->get('reports/comparativoMensual', 'ReportsAnalytics::comparativoMensual');
$routes->get('reports/comparativoMensualPdf', 'ReportsAnalytics::comparativoMensualPdf');
$routes->get('reports/comparativoMensualExcel', 'ReportsAnalytics::comparativoMensualExcel');
$routes->get('giftcards', 'ModulePlaceholder::index');
$routes->get('employees', 'Employees::index');
$routes->get('employees/view/(:num)', 'Employees::view/$1');
$routes->get('employees/view', 'Employees::view/-1');
$routes->post('employees/save/(:num)', 'Employees::save/$1');
$routes->post('employees/save/(:any)', 'Employees::save/-1');
$routes->post('employees/save', 'Employees::save');
$routes->post('employees/search', 'Employees::search');
$routes->post('employees/suggest', 'Employees::suggest');
$routes->post('employees/delete', 'Employees::delete');
$routes->post('employees/toggleStatus', 'Employees::toggleStatus');
$routes->get('employees/checkSession', 'Employees::checkSession');
$routes->get('config', 'Config::index');
$routes->get('config/pdf-templates', 'PdfResultTemplates::index');
$routes->get('config/pdf-templates/edit/(:num)', 'PdfResultTemplates::edit/$1');
$routes->post('config/pdf-templates/save', 'PdfResultTemplates::save');
$routes->post('config/pdf-templates/create', 'PdfResultTemplates::create');
$routes->post('config/pdf-templates/duplicate/(:num)', 'PdfResultTemplates::duplicate/$1');
$routes->get('config/pdf-templates/delete/(:num)', 'PdfResultTemplates::delete/$1');
$routes->get('config/sobres/edit/(:num)', 'EnvelopeTemplates::edit/$1');
$routes->post('config/sobres/save', 'EnvelopeTemplates::save');
$routes->post('config/sobres/preview-markup', 'EnvelopeTemplates::previewMarkup');
$routes->post('config/sobres/create', 'EnvelopeTemplates::create');
$routes->get('config/sobres/delete/(:num)', 'EnvelopeTemplates::delete/$1');
$routes->get('config/sobres/set-active/(:num)', 'EnvelopeTemplates::setActive/$1');
$routes->post('config/sobres/set-print-template', 'EnvelopeTemplates::setPrintTemplate');
$routes->get('config/sobres/media/(:num)/(:segment)', 'EnvelopeTemplates::media/$1/$2');
$routes->get('config/backup', 'Config::backup');
$routes->get('config/backupTenants', 'Config::backupTenants');
$routes->get('cron/tenant-backup-schedule', 'Cron::tenantBackupSchedule');
$routes->get('cron/writable-cache-purge', 'Cron::writableCachePurge');
$routes->get('config/opciones', 'Config::opciones');
$routes->get('config/exportOpciones', 'Config::exportOpciones');
$routes->get('config/exportOpcion/(:num)', 'Config::exportOpcion/$1');
$routes->post('config/save', 'Config::save');
$routes->post('config/saveLabValidation', 'Config::saveLabValidation');
$routes->post('config/uploadLabApproverSeal', 'Config::uploadLabApproverSeal');
$routes->post('config/uploadLabApproverSignature', 'Config::uploadLabApproverSignature');
$routes->post('config/deleteLabApproverSeal', 'Config::deleteLabApproverSeal');
$routes->post('config/deleteLabApproverSignature', 'Config::deleteLabApproverSignature');
$routes->post('config/saveUiStyle', 'Config::saveUiStyle');
$routes->post('config/saveComprobanteStyle', 'Config::saveComprobanteStyle');
$routes->get('config/exportComprobanteStyle', 'Config::exportComprobanteStyle');
$routes->post('config/importComprobanteStyle', 'Config::importComprobanteStyle');
$routes->post('config/saveWhatsapp', 'Config::saveWhatsapp');
$routes->post('config/saveDeliveryNotifications', 'Config::saveDeliveryNotifications');
$routes->post('config/saveSin', 'Config::saveSin');
$routes->post('config/saveInstitucionDescuento', 'Config::saveInstitucionDescuento');
$routes->post('config/savepoblacion', 'Config::savePoblacion');
$routes->post('config/saveopcion', 'Config::saveOpcion');
$routes->post('config/importOpciones', 'Config::importOpciones');
$routes->post('config/importOpcion/(:num)', 'Config::importOpcion/$1');
$routes->post('config/saveopcionvalor', 'Config::saveOpcionValor');
$routes->post('config/reorderopcionvalores', 'Config::reorderOpcionValores');
$routes->post('config/transformopcionnombres', 'Config::transformOpcionNombres');
$routes->post('config/transformtiposmuestra', 'Config::transformTiposMuestra');
$routes->post('config/transformmetodos', 'Config::transformMetodos');
$routes->post('config/savevalortabla', 'Config::saveValorTabla');
$routes->post('config/savetenant', 'Config::saveTenant');
$routes->post('config/saveTenantBackupSchedule', 'Config::saveTenantBackupSchedule');
$routes->post('config/saveWritableCacheSchedule', 'Config::saveWritableCacheSchedule');
$routes->post('config/clearWritableCache', 'Config::clearWritableCache');
$routes->post('config/closeAllSessions', 'Config::closeAllSessions');
$routes->post('config/testSin', 'Config::testSin');
$routes->post('config/getActiveSessions', 'Config::getActiveSessions');
$routes->post('config/getSessionsVersion', 'Config::getSessionsVersion');
$routes->post('config/killSession', 'Config::killSession');
$routes->get('config/debugSessions', 'Config::debugSessions');
$routes->get('config/deletepoblacion/(:num)', 'Config::deletePoblacion/$1');
$routes->get('config/deleteInstitucionDescuento', 'Config::deleteInstitucionDescuento');
$routes->post('config/savetipomuestra', 'Config::saveTipoMuestra');
$routes->get('config/deletetipomuestra/(:num)', 'Config::deleteTipoMuestra/$1');
$routes->post('config/savegenero', 'Config::saveGenero');
$routes->get('config/deletegenero/(:num)', 'Config::deleteGenero/$1');
$routes->post('config/savemetodo', 'Config::saveMetodo');
$routes->get('config/deletemetodo/(:num)', 'Config::deleteMetodo/$1');
$routes->post('config/saveleyendacultivo', 'Config::saveLeyendaCultivo');
$routes->get('config/deleteleyendacultivo/(:num)', 'Config::deleteLeyendaCultivo/$1');
$routes->post('config/saveleyendacultivocategoria', 'Config::saveLeyendaCultivoCategoria');
$routes->get('config/deleteleyendacultivocategoria/(:num)', 'Config::deleteLeyendaCultivoCategoria/$1');
$routes->post('config/savefichaclinica', 'Config::saveFichaClinica');
$routes->get('config/deletefichaclinica/(:num)', 'Config::deleteFichaClinica/$1');
$routes->post('config/savefichaclinicamatriz', 'Config::saveFichaClinicaMatriz');
$routes->get('config/exportfichaclinica/(:num)', 'Config::exportFichaClinica/$1');
$routes->get('config/exportfichaclinicamatriz/(:num)', 'Config::exportFichaClinicaMatriz/$1');
$routes->post('config/importfichaclinica', 'Config::importFichaClinica');
$routes->post('config/importfichaclinicamatriz/(:num)', 'Config::importFichaClinicaMatriz/$1');
$routes->post('config/savefichaclinicapruebas', 'Config::saveFichaClinicaPruebas');
$routes->get('config/deleteopcion/(:num)', 'Config::deleteOpcion/$1');
$routes->get('config/deleteopcionvalor/(:num)', 'Config::deleteOpcionValor/$1');
$routes->get('config/deletevalortabla/(:alpha)/(:num)', 'Config::deleteValorTabla/$1/$2');
$routes->get('config/deletetenant/(:num)', 'Config::deleteTenant/$1');
$routes->post('config/saveTenantHomeBroadcast', 'Config::saveTenantHomeBroadcast');
$routes->post('config/saveTenantSubscriptionAlertDays', 'Config::saveTenantSubscriptionAlertDays');
$routes->post('config/saveTenantSubscriptionPayment', 'Config::saveTenantSubscriptionPayment');
$routes->post('config/uploadTenantSubscriptionVoucher/(:num)', 'Config::uploadTenantSubscriptionVoucher/$1');
$routes->get('config/deleteTenantSubscriptionPayment/(:num)', 'Config::deleteTenantSubscriptionPayment/$1');
$routes->get('config/tenantSubscriptionVoucher/(:num)', 'Config::downloadTenantSubscriptionVoucher/$1');
$routes->post('config/provisiontenant/(:num)', 'Config::provisionTenant/$1');
$routes->post('config/migrateCurrentDatabase', 'Config::migrateCurrentDatabase');
$routes->post('config/migrateAllTenants', 'Config::migrateAllTenants');
$routes->match(['get', 'post'], 'config/ghostEnterTenant/(:num)', 'Config::ghostEnterTenant/$1');
$routes->get('config/ghostExitTenant', 'Config::ghostExitTenant');
$routes->get('seguridad', 'ModulePlaceholder::index');
$routes->get('seguridad_aforo', 'ModulePlaceholder::index');
$routes->get('controlcalidad', 'Controlcalidad::index');
$routes->get('controlcalidad/grafica/(:num)', 'Controlcalidad::grafica/$1');
$routes->get('controlcalidad/graficaPdf/(:num)', 'Controlcalidad::graficaPdf/$1');
$routes->post('controlcalidad/savecontrol', 'Controlcalidad::savecontrol');
$routes->post('controlcalidad/savevalor', 'Controlcalidad::savevalor');
$routes->get('reactivos', 'Reactivos::index');
$routes->get('reactivos/kardex', 'Reactivos::kardex');
$routes->get('reactivos/kardexPdf', 'Reactivos::kardexPdf');
$routes->get('inventario', 'Reactivos::index');
$routes->get('inventario/kardex', 'Reactivos::kardex');
$routes->get('inventario/kardexPdf', 'Reactivos::kardexPdf');
$routes->get('reactivos/editar/(:num)', 'Reactivos::editar/$1');
$routes->get('reactivos/lotes/(:num)', 'Reactivos::lotes/$1');
$routes->post('reactivos/savereactivo', 'Reactivos::savereactivo');
$routes->post('reactivos/savelote', 'Reactivos::savelote');
$routes->post('reactivos/registrarsalida', 'Reactivos::registrarsalida');
$routes->post('reactivos/revertirSalida', 'Reactivos::revertirSalida');
$routes->post('reactivos/eliminar/(:num)', 'Reactivos::eliminar/$1');
$routes->get('equipos', 'Equipos::index');
$routes->get('equipos/detalle/(:num)', 'Equipos::detalle/$1');
$routes->get('equipos/historialPrint/(:num)', 'Equipos::historialPrint/$1');
$routes->get('equipos/historialPdf/(:num)', 'Equipos::historialPdf/$1');
$routes->post('equipos/saveequipo', 'Equipos::saveequipo');
$routes->post('equipos/savemantenimiento', 'Equipos::savemantenimiento');
$routes->get('leyendas', 'Leyendas::index');
$routes->post('leyendas/save', 'Leyendas::save');
$routes->get('leyendas/delete/(:num)', 'Leyendas::delete/$1');
$routes->get('egresos', 'Egresos::index');
$routes->post('egresos/save', 'Egresos::save');
$routes->get('egresos/delete/(:num)', 'Egresos::delete/$1');
$routes->get('auditoria', 'Auditoria::index');
$routes->get('lab-modern/trazabilidad', 'LabModern::trazabilidad');
$routes->get('lab-modern/procesos', 'LabModern::procesos');
$routes->get('lab-modern/trazabilidad-data', 'LabModern::trazabilidadData');
$routes->get('doctors', 'Doctors::index');
$routes->get('doctor_commissions', 'DoctorCommissions::index');
$routes->get('doctor_commissions/details/(:num)', 'DoctorCommissions::details/$1');
$routes->get('doctor_commissions/pay/(:num)', 'DoctorCommissions::pay/$1');
$routes->get('doctor_commissions/pay/bulk/(:num)', 'DoctorCommissions::bulkPay/$1');
$routes->post('doctor_commissions/processPayment', 'DoctorCommissions::processPayment');
$routes->post('doctor_commissions/bulkPay', 'DoctorCommissions::processBulkPayment');
$routes->post('doctor_commissions/delete/(:num)', 'DoctorCommissions::delete/$1');
$routes->get('labotests', 'Labotests::index');
$routes->get('labotests/opciones', 'Labotests::opciones');
$routes->post('labotests/saveopcion', 'Labotests::saveOpcion');
$routes->post('labotests/saveopcionvalor', 'Labotests::saveOpcionValor');
$routes->post('labotests/savevalortabla', 'Labotests::saveValorTabla');
$routes->get('labotests/deleteopcion/(:num)', 'Labotests::deleteOpcion/$1');
$routes->get('labotests/deleteopcionvalor/(:num)', 'Labotests::deleteOpcionValor/$1');
$routes->get('labotests/deletevalortabla/(:alpha)/(:num)', 'Labotests::deleteValorTabla/$1/$2');
$routes->get('labotests/view/(:num)', 'Labotests::view/$1');
$routes->get('labotests/view', 'Labotests::view/-1');
$routes->get('labotests/subview/(:num)', 'Labotests::subview/$1');
$routes->get('labotests/detail/(:num)', 'Labotests::detail/$1');
$routes->get('labotests/perfiles', 'Labotests::perfiles');
$routes->post('labotests/saveperfil', 'Labotests::saveperfil');
$routes->get('labotests/deleteperfil/(:num)', 'Labotests::deleteperfil/$1');
$routes->post('labotests/save/(:num)', 'Labotests::save/$1');
$routes->post('labotests/save/(:any)', 'Labotests::save/0');
$routes->post('labotests/save', 'Labotests::save');
$routes->post('labotests/savesubmain/(:num)', 'Labotests::savesubmain/$1');
$routes->post('labotests/savesub', 'Labotests::savesub');
$routes->post('labotests/savesecitem', 'Labotests::savesecitem');
$routes->post('labotests/savecultivomatriz', 'Labotests::saveCultivoMatriz');
$routes->post('labotests/savepersonalizadomatriz', 'Labotests::savePersonalizadoMatriz');
$routes->post('labotests/orderSecItems', 'Labotests::orderSecItems');
$routes->post('labotests/orderPriResultados', 'Labotests::orderPriResultados');
$routes->post('labotests/sortsecitemsbycriteria', 'Labotests::sortsecitemsbycriteria');
$routes->post('labotests/sortpriresultadosbycriteria', 'Labotests::sortpriresultadosbycriteria');
$routes->post('labotests/transformsecitemnames', 'Labotests::transformsecitemnames');
$routes->post('labotests/transformpriresultadosumedida', 'Labotests::transformpriresultadosumedida');
$routes->post('labotests/reorderanalysis', 'Labotests::reorderAnalysis');
$routes->post('labotests/sortanalysisalphabetic', 'Labotests::sortAnalysisAlphabetic');
$routes->post('labotests/reordercategories', 'Labotests::reorderCategories');
$routes->get('labotests/duplicados', 'Labotests::duplicateAnalysesReport');
$routes->post('labotests/transformnames', 'Labotests::transformNames');
$routes->post('labotests/duplicateanalysis', 'Labotests::duplicateAnalysisToParent');
$routes->post('labotests/saveformula', 'Labotests::saveFormula');
$routes->post('labotests/deleteformula/(:num)', 'Labotests::deleteFormula/$1');
$routes->post('labotests/savereactivoconsumo', 'Labotests::saveReactivoConsumo');
$routes->post('labotests/deletereactivoconsumo/(:num)', 'Labotests::deleteReactivoConsumo/$1');
$routes->get('labotests/duplicatesecitem/(:num)', 'Labotests::duplicatesecitem/$1');
$routes->post('labotests/duplicatesecitem/(:num)', 'Labotests::duplicatesecitem/$1');
$routes->get('labotests/exportdetailconfig/(:num)', 'Labotests::exportdetailconfig/$1');
$routes->post('labotests/importdetailconfig/(:num)', 'Labotests::importdetailconfig/$1');
$routes->get('labotests/deletecategory/(:num)', 'Labotests::deletecategory/$1');
$routes->get('labotests/prianacategoriadeletepreview/(:num)', 'Labotests::prianacategoriaDeletePreview/$1');
$routes->post('labotests/deleteprianacategoria/(:num)', 'Labotests::deleteprianacategoria/$1');
$routes->get('labotests/deleteprianacategoria/(:num)', 'Labotests::deleteprianacategoria/$1');
$routes->get('labotests/manuales', 'Labotests::manualesIndex');
$routes->get('labotests/manuales/(:num)', 'Labotests::manuales/$1');
$routes->post('labotests/savemanual', 'Labotests::saveManual');
$routes->get('labotests/deletemanual/(:num)', 'Labotests::deleteManual/$1');
$routes->get('labotests/recomendacionesprevias/(:num)', 'Labotests::recomendacionesPrevias/$1');
$routes->post('labotests/saverecomendacionesprevias', 'Labotests::saveRecomendacionesPrevias');
$routes->get('interpretacion-clinica', 'InterpretacionClinica::index');
$routes->get('interpretacion-clinica/(:num)', 'InterpretacionClinica::detail/$1');
$routes->post('interpretacion-clinica/save', 'InterpretacionClinica::save');
$routes->post('labotests/deletesecitem/(:num)', 'Labotests::deletesecitem/$1');
$routes->get('labotests/deletesecitem/(:num)', 'Labotests::deletesecitem/$1');
$routes->post('labotests/deletesecitemsbulk', 'Labotests::deletesecitemsbulk');
$routes->post('labotests/duplicatesecitemsbulk', 'Labotests::duplicatesecitemsbulk');
$routes->post('labotests/expandreferenciasbygenerossec', 'Labotests::expandreferenciasbygenerossec');
$routes->post('labotests/expandreferenciasbygenerospri', 'Labotests::expandreferenciasbygenerospri');
$routes->post('labotests/savepriresultado', 'Labotests::savepriresultado');
$routes->post('labotests/deletepriresultado/(:num)', 'Labotests::deletepriresultado/$1');
$routes->get('labotests/deletepriresultado/(:num)', 'Labotests::deletepriresultado/$1');
$routes->post('labotests/deletepriresultadosbulk', 'Labotests::deletepriresultadosbulk');
$routes->post('labotests/duplicatepriresultadosbulk', 'Labotests::duplicatepriresultadosbulk');
$routes->get('toquotes', 'Toquotes::index');
$routes->get('toquotes/search', 'Toquotes::search');
$routes->post('toquotes/savetoquotelog', 'Toquotes::savetoquotelog');
$routes->post('toquotes/exportPdf', 'Toquotes::exportPdf');
$routes->get('toquotes/exportPdfById/(:num)', 'Toquotes::exportPdfById/$1');
$routes->get('toquotes/exportPdfById/(:num)/(:alpha)', 'Toquotes::exportPdfById/$1/$2');
$routes->get('registers', 'Registers::index');
$routes->get('expediente', 'Expediente::index');
$routes->get('expediente/view/(:num)', 'Expediente::view/$1');
$routes->get('expediente/chart/(:num)', 'Expediente::chart/$1');
$routes->post('expediente/search', 'Expediente::search');
$routes->get('registers/csrfRefresh', 'Registers::csrfRefresh');
$routes->get('registers/lista', 'Registers::lista');
$routes->get('registers/anulada/(:num)', 'Registers::anulada/$1');
$routes->get('registers/view/(:num)', 'Registers::view/$1');
$routes->get('registers/edit/(:num)', 'Registers::edit/$1');
$routes->get('registers/orden/(:num)', 'Registers::orden/$1');
$routes->get('registers/ordenPdf/(:num)', 'Registers::ordenPdf/$1');
$routes->get('registers/insumos/(:num)', 'Registers::insumos/$1');
$routes->get('registers/viewreport/(:num)', 'Registers::viewreport/$1');
$routes->get('registers/printreport/(:num)', 'Registers::printreport/$1');
$routes->get('registers/printEnvelope/(:num)', 'Registers::printEnvelope/$1');
$routes->get('registers/pdf/(:num)', 'Registers::pdf/$1');
$routes->get('registers/qrResultadosPng/(:num)', 'Registers::qrResultadosPng/$1');
$routes->get('registers/viewcomprobante/(:num)', 'Registers::viewcomprobante/$1');
$routes->get('registers/printcomprobante/(:num)', 'Registers::printcomprobante/$1');
$routes->get('registers/comprobantePdf/(:num)', 'Registers::comprobantePdf/$1');
$routes->post('registers/save', 'Registers::save');
$routes->post('registers/update/(:num)', 'Registers::update/$1');
$routes->post('registers/validar', 'Registers::validar');
$routes->post('registers/crearmuestra', 'Registers::crearmuestra');
$routes->get('registers/cambiarestadomuestra/(:num)/(:num)', 'Registers::cambiarestadomuestra/$1/$2');
$routes->post('registers/search_paciente', 'Registers::searchPaciente');
$routes->post('registers/search_doctor', 'Registers::searchDoctor');
$routes->post('registers/search_prueba', 'Registers::searchPrueba');
$routes->post('registers/saveregvalues', 'Registers::saveregvalues');
$routes->get('registers/fichaClinicaForm', 'Registers::fichaClinicaForm');
$routes->post('registers/saveFichaClinica', 'Registers::saveFichaClinica');
$routes->post('registers/saveanalisiss', 'Registers::saveanalisiss');
$routes->post('registers/delete/(:num)', 'Registers::delete/$1');
$routes->post('registers/addpago/(:num)', 'Registers::addpago/$1');
$routes->post('registers/editarAbonoPago/(:num)', 'Registers::editarAbonoPago/$1');
$routes->post('registers/eliminarAbonoPago/(:num)', 'Registers::eliminarAbonoPago/$1');
$routes->get('registers/historial/(:num)', 'Registers::historial/$1');
$routes->post('registers/sendWhatsapp/(:num)', 'Registers::sendWhatsapp/$1');
$routes->get('registers/deliveryNotifications', 'Registers::deliveryNotifications');
$routes->post('registers/notifyDelivery/(:num)', 'Registers::notifyDelivery/$1');
$routes->get('registers/servePdfFile/(:alphanum)', 'Registers::servePdfFile/$1');
$routes->get('customers/view/(:num)', 'Customers::view/$1');
$routes->get('customers/view', 'Customers::view/0');
$routes->post('customers/search', 'Customers::search');
$routes->post('customers/suggest', 'Customers::suggest');
$routes->post('customers/save/(:num)', 'Customers::save/$1');
$routes->post('customers/save/(:any)', 'Customers::save/-1');
$routes->post('customers/save', 'Customers::save');
$routes->post('customers/delete', 'Customers::delete');
$routes->post('customers/transformnames', 'Customers::transformNames');

// Doctors
$routes->get('doctors/view/(:num)', 'Doctors::view/$1');
$routes->get('doctors/view', 'Doctors::view/-1');
$routes->get('doctors/save/(:any)', 'Doctors::view/$1');
$routes->get('doctors/save', 'Doctors::view/-1');
$routes->post('doctors/search', 'Doctors::search');
$routes->post('doctors/suggest', 'Doctors::suggest');
$routes->post('doctors/saves/(:num)', 'Doctors::saves/$1');
$routes->post('doctors/save/(:any)', 'Doctors::saves/$1');
$routes->post('doctors/save', 'Doctors::saves');
// GET evita 404 si se abre el href del botón Borrar sin POST (anchor); la eliminación real sigue siendo POST desde JS
$routes->get('doctors/delete/(:num)', 'Doctors::deleteRedirect/$1');
$routes->get('doctors/delete', 'Doctors::deleteRedirect');
$routes->post('doctors/delete', 'Doctors::delete');
$routes->post('doctors/transformnames', 'Doctors::transformNames');
