...
$cellBase = 'line-height:' . esc((string) $lh, 'attr')
    ...
    . ';' . $borderBottom;
$leftPad  = $mpdfFooterRow ? 'padding-left:0;' : ('padding-left:' . esc((string) $ml, 'attr') . 'mm;padding-right:0;');
$rightPad = $mpdfFooterRow ? 'padding-right:0;' : ('padding-right:' . esc((string) $mr, 'attr') . 'mm;padding-left:0;');
...
<?php if ($mpdfFooterAboveTable && $nCols === 3): ?>
        <td class="pdf-order-sheet-header-patient pdf-cell pdf-cell--left mpdf-order-sheet-patient" align="left" width="40%" style="<?= esc($cellBase . 'text-align:left !important;padding:0;width:40%;max-width:40%;box-sizing:border-box;', 'attr') ?>"><?= esc($patientLine) ?></td>
        ...
        <td class="pdf-order-sheet-header-orden pdf-cell pdf-cell--right mpdf-order-sheet-order" align="right" width="40%" style="<?= esc($cellBase . 'text-align:right !important;padding:0;width:40%;max-width:40%;box-sizing:border-box;', 'attr') ?>"><?= esc($orderLine) ?></td>
<?php else: ?>
        <td class="pdf-order-sheet-header-patient pdf-cell pdf-cell--left mpdf-order-sheet-patient" colspan="<?= $leftSpan ?>" align="left" style="<?= esc($leftStyle, 'attr') ?>"><?= esc($patientLine) ?></td>
        <td class="pdf-order-sheet-header-orden pdf-cell pdf-cell--right mpdf-order-sheet-order" colspan="<?= $rightSpan ?>" align="right" style="<?= esc($rightStyle, 'attr') ?>"><?= esc($orderLine) ?></td>
<?php endif; ?>
    </tr>
<?php
if ($mpdfFromPageTwo) {
