...
{	
	...
	// Objeto (no string) para que .load() use POST; serializeArray incluye el token CSRF
	var postData = {};
	$('#search_form').serializeArray().forEach(function(item) {
		postData[item.name] = item.value;
	});
		
	$('#sortable_table tbody').load($('#search_form').attr('action'), postData, function()
	{
		...
		if (typeof tb_init === 'function') tb_init('#sortable_table a.thickbox');
	...
	$.post(url, { 'ids[]': row_ids },function(response)
	{
		...
		if(response.success)
		{
			...
			for(index in response.ids) {
                ...
                (response.ids[index],url.replace(/[^\/]+$/,'get_row'));
			}
...
//        alert(selected_item_ids);
	...
	//		$(this).blur();
    //   json = JSON.stringify(selected_item_ids);
     window.location.href='index.php/items/bulk_edit/?items='+selected_item_ids; 
		}
		else
		{
			alert(none_selected_message);
...
function reinit_row(checkbox_id)
{
	...
	update_sortable_table();
	if (typeof tb_init === 'function') tb_init(new_row.find("a.thickbox"));
	//re-enable e-mail
